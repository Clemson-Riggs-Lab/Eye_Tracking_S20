...
import DataQuality_correctedCoords
...
def missingDataCheck(tracker_type,df,file):
        ...
        ErrLogName=(str(file)).split('_')
        ErrLogName=ErrLogName[0]+"ErrLog.txt"
        output_file = open(str(ErrLogName), "w")
        output_file.write(
        'File name:'+str(file)+'\n')
        output_file.write(
        '-------------------------------------------------------------------------'+'\n')
...
def DeleteErrors(combined_list,missing_packets,df):
        ...
        return df
#@Sam: added this when exploring lag issue. has no real practical use    
def AOIDetermination(t, df):
    if t==1:
         AOI=[]
         for i in range(0,len(df)):
             x = df['BestPogX'][i]
             y = df['BestPogY'][i]  
             if 0<= x <=910 and 0<= y <=905:
                 AOI.append('AOI 3: Map')
             elif 0<=x<=910 and 930<=y<=1440:
                AOI.append('AOI 2: Reroute Menu')
             elif 930<=x<=2560 and 0<=y<813:
                AOI.append('AOI 1: Video Feeds')
             elif 930<=x<=2560 and 835<=y<=1158:
                AOI.append('AOI 4: General Health panel')
             elif 930<=x<=2560 and 1180<=y<=1440:
                AOI.append('AOI 5: Chat Message Panel')
             else:
                #a special case if the gazepoint lands on any of the grey or black distances seperating each AOI
                AOI.append('Gaze point is not in any of the designated AOIs')
         df["AOI"] = AOI
         return df
...
xError = Entry(frame9) #error for horizontal direction
xError.pack(side=LEFT)

text9 = Label(frame10, text='Enter yError (only for Gazepoint):                                         ')
text9.pack(side=LEFT)
yError = Entry(frame10) #error for vertical direction
